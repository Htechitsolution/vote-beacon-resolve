...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (isResultEmail && resultUrl) {
      // Results email
      emailSubject = `Voting Results for ${projectName} - ${resultTitle || "Meeting"}`;
      emailHtml = `
      ...
      // Voting link email with OTP
      emailSubject = `Your Voting Link and OTP for ${projectName} Meeting - ${otp}`;
      emailHtml = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
      ...
      // Default OTP email
      emailSubject = `Your OTP for ${projectName} Meeting - ${otp}`;
      emailHtml = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
